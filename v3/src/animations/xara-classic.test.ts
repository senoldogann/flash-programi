...
import { animationShowsAlternateFace, evaluateAnimation } from './evaluator';
...
describe('classic Xara-style nick animation', () => {
  it('turns the text edge-on and exposes its back face during a double-sided rotation', () => {
    const definition = animation('xara-double-sided');
    const front = evaluateAnimation(definition, 0);
    const edge = evaluateAnimation(definition, 400);
    const back = evaluateAnimation(definition, 800);

    expect(front.scaleX).toBeCloseTo(1, 3);
    expect(animationShowsAlternateFace(definition, 0)).toBe(false);
    expect(Math.abs(edge.scaleX)).toBeLessThan(0.08);
    expect(back.scaleX).toBeCloseTo(1, 3);
    expect(animationShowsAlternateFace(definition, 800)).toBe(true);
  });
});
