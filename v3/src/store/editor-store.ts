...
export const useEditorStore = create<EditorStore>((set, get) => ({
  ...
  selectElement: (id) => {
    ...
    if (id !== null && !project.elements.some((element) => element.id === id)) {
      return;
    }
  ...
  addImage: (assetUrl, naturalWidth, naturalHeight) => {
    if (!assetUrl) {
      throw new Error('Fotoğraf adresi boş olamaz.');
    }
  ...
  removeElement: (id) => {
    ...
    if (!state.project.elements.some((element) => element.id === id)) {
      return;
    }
  ...
  commitTransform: (beforeProject, afterProject) => {
    ...
    if (projectsEqual(beforeProject, validatedAfter)) {
      set({ project: cloneProject(validatedAfter) });
      return;
    }
